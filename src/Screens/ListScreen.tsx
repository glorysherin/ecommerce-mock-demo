...
import { ITEMS, IMAGE_URL } from '../StaticData';

export default function ListScreen() {
    // const titlesArray = ["Title 1", "Title 2", "Title 3", "Title 4", "Title 5", "Title 6", "Title 7", "Title 8", "Title 9", "Title 10", "Title 11", "Title 12"]
    ...
    useEffect(() => {
        const loadData = async () => {
            const savedData = await AsyncStorage.getItem("countsStorage");
            if (savedData !== null) {
                setCount(JSON.parse(savedData))
            }
        };
        loadData();
    }, [])

    const saveCount = async (updated: number[]) => {
        await AsyncStorage.setItem("countsStorage", JSON.stringify(updated));
    };
    return (
        <View style = {{flex: 1}}>
